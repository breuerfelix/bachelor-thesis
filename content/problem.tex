...
% TODO evtl scherfälligkeit von tcp erwähnen
...
In diesem Szenario muss der Node, auf dem der Client verbunden war, alle persistierten Daten an den neuen Node übermitteln.
Wenn der Client über einen längeren Zeitraum nicht verbunden war, können sich viele Daten anhäufen.
\\
...
\\
Da \ac{mqtt} ein zustandbehaftetes und verbindungsorientiertes Protokoll ist, kann die load balancing Entscheidung nicht auf Paket-, sondern nur auf Verbindungsebene getroffen werden. Sobald eine Clientverbindung aufgebaut ist, werden alle Nachrichten des Clients immer an denselben Node weitergeleitet.
...
% TODO
Die Einführung von \textit{Shared Subscriptions} (siehe Kapitel \ref{s:shared-subscriptions})
\\
