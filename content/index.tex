...
\begin{lstlisting}[language=yaml, label={org-dc}, caption=Original Docker-Compose]
version: '2.1'
services:
...
\begin{lstlisting}[language=conf, caption=nginx.no-ssl.conf]
server {
  listen 80;
...
\end{lstlisting}
\begin{lstlisting}[language=conf, caption=nginx.ssl.conf]
...
\begin{lstlisting}[language=bash, label={entrypointsh}, caption=entrypoint.sh]
#!/bin/sh
...
\begin{lstlisting}[language=yaml, caption=Restart Policy]
# docker compose
version: '3'
services:
  mender-useradm:
...
\begin{lstlisting}[language=yaml, caption=Service Name Alias]
# docker compose
version: '3'
services:
  mender-deployments:
...
\begin{lstlisting}[language=bash, label={deploy-mender}, caption=Docker Swarm Deploy]
...
\begin{lstlisting}[language=yaml, label={k8s-device-auth}, caption=Device Auth Service]
apiVersion: apps/v1
kind: Deployment
metadata:
...
\begin{lstlisting}[language=conf, label={network-dhcp}, caption=20-dhcp.network]
[Match]
Name=en*
[Network]
DHCP=yes
...
\begin{lstlisting}[language=conf, label={systemd-append}, caption=systemd\_\%.bbappend]
...
\begin{lstlisting}[language=conf, label={linux-append}, caption=linux-\%.bbappend]
include ${@bb.utils.contains(
  'DISTRO_FEATURES', 'virtualization',
  'virtualization.inc', '', d
)}
\end{lstlisting}
\begin{lstlisting}[language=conf, label={virtualization-inc}, caption=virtualization.inc]
FILESEXTRAPATHS_prepend := "${THISDIR}/linux-yocto:"

SRC_URI += "file://docker.scc"
\end{lstlisting}
\begin{lstlisting}[language=conf, label={docker-scc}, caption=docker.scc]
...
\begin{lstlisting}[language=conf, label={dns}, caption=resolved.conf]
[Resolve]
DNS=1.1.1.1
FallbackDNS=1.0.0.1
LLMNR=no
\end{lstlisting}
\begin{lstlisting}[language=conf, label={ntp}, caption=timsyncd.conf]
[Time]
NTP=pool.ntp.org
FallbackNTP=de.pool.ntp.org
\end{lstlisting}
\begin{lstlisting}[language=conf, label={systemd-append-advanced}, caption=systemd\_\%.bbappend]
PACKAGECONFIG_append = " networkd resolved"
...
\begin{lstlisting}[language=conf, label={local-conf}, caption=local.conf]
INHERIT += "mender-full"
...
\begin{lstlisting}[language=yaml, label={gitlab-ci}, caption=.gitlab-ci.yml]
stages:
  - build
  - build_qemu
  - deploy
...
\begin{lstlisting}[language=bash, label={run-sh}, caption=run.sh]
#!/bin/bash
